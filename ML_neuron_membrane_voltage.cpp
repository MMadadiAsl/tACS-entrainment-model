...
float Vm1[T], n1[T], Vm2[T], n2[T], m1_inf, n1_inf, tau1_n, m2_inf, n2_inf, tau2_n;
...
//*******************************************************************************************
	...
	for (t = 0; t < T - 1; t++){
		...
		outfile1 << t * dt << '\t' << Vm1[t] << '\t' << n1[t] << endl;
		outfile2 << t * dt << '\t' << Vm2[t] << '\t' << n2[t] << endl;
	}

	cout << '\n' << endl;
	cout << "Type-I  mean firing rate = " << MeanFiringRate(num1_spikes,T,dt)  << " Hz" << endl;
	cout << "Type-II mean firing rate = " << MeanFiringRate(num2_spikes,T,dt)  << " Hz" << endl;
	cout << '\n' << endl;
